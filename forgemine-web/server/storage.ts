...
/**
 * Download file content directly from storage using the API
 * This bypasses the CloudFront URL which may have access restrictions
 */
export async function storageDownload(relKey: string): Promise<Buffer> {
  const { baseUrl, apiKey } = getStorageConfig();
  const key = normalizeKey(relKey);
  
  // Use the download endpoint to get the file content directly
